    ...
    public class OrdersController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetOrders([FromQuery] OrdersParametersModel parameters)
        {
            ...
            var orders = _orderApiService.GetOrders(parameters.Ids, parameters.CreatedAtMin,
                                                    ...
                                                    parameters.CustomerId, storeId, parameters.OrderByDateDesc);
            ...
            Delta<OrderDto> orderDelta)
        {
            ...
            IList<ShoppingCartItem> cart = await _shoppingCartService.GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, newOrder.StoreId);

            int orderQuantity = cart.Count / 3 + (cart.Count % 3 > 0 ? 1 : 0);
            ...
            {
                //Get a cart section
                IList<ShoppingCartItem> cartSection = cart.Skip(i * 3).Take(3).ToList();

                //Fill cart with cartSection
                foreach (var sci in cartSection)
                {
                    var product = await _productService.GetProductByIdAsync(sci.ProductId);
                    await _shoppingCartService.AddToCartAsync(customer, product, ShoppingCartType.ShoppingCart, newOrder.StoreId);
                }
        ...
        private async Task<PlaceOrderResult> PlaceOrderAsync(Order newOrder, Customer customer)
        {
            ...
            if (!string.IsNullOrEmpty(newOrder.CustomValuesXml))
            {
                paymentData = JsonSerializer.Deserialize<Dictionary<string, object>>(newOrder.CustomValuesXml);
            }
