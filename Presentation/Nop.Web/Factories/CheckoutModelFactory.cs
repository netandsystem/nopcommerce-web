...
namespace Nop.Web.Factories
{
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        /// Prepare checkout completed model
        /// </summary>
        /// <param name="orders">IList<Order></param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the checkout completed model
        /// </returns>
        public virtual Task<CheckoutCompletedModelList> PrepareCheckoutCompletedModelAsync(IList<Order> orders)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));

            if (orders.Count == 0)
                throw new ArgumentException("Orders list is empty", nameof(orders));

            IList<CheckoutCompletedModel> modelList = new List<CheckoutCompletedModel>();

            foreach (var order in orders)
            {
                var checkoutCompletedModel = new CheckoutCompletedModel
                {
                    OrderId = order.Id,
                    OnePageCheckoutEnabled = _orderSettings.OnePageCheckoutEnabled,
                    CustomOrderNumber = order.CustomOrderNumber
                ...
                modelList.Add(checkoutCompletedModel);
            }

            var model = new CheckoutCompletedModelList
            {
                OrdersModelList = modelList
            };
