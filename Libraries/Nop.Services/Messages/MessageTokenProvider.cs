...
#nullable enable
    ...
    public virtual void AddReportTokensAsync(Report report, IList<Token> tokens, MessageTemplate messageTemplate)
    {
        if (report.DataDic is null)
        {
            throw new NopException("Report data is null in AddReportTokensAsync");
        }

        //ReportId
        AddCommonTokens(messageTemplate, report, tokens);

        foreach (var item in report.DataDic)
        {
            if (item.FeatureValue != null)
            {
                tokens.Add(new Token(item.Key, item.FeatureValue));
            }
            else if (item.StringValue != null)
            {
                tokens.Add(new Token(item.Key, item.StringValue));
            }
            else if (item.TableValue != null)
            {
                ...
                tokens.Add(new Token(item.Key, result, true));
            }
            else if (item.StyledTableValue != null)
            {
                var sb = new MyStringBuilder();

                GetStyledTable(item.Label, item.StyledTableValue, sb);
        ...
        SurroundTemplate(messageTemplate);
    }

    private string? GetInnerHtmlById(string html, string id)
    {
        // Crea un analizador HTML
        var parser = new HtmlParser();

        // Parsea el HTML
        var document = parser.ParseDocument(html);

        // Encuentra el elemento con el ID
        var tag = document.GetElementById(id);

        return tag?.InnerHtml;
    }

    private class HtmlTokenConst
    {
        public string Id { get; set; } = "";
        public string Span { get => $"<span id=\"{Id}\"></span>"; }
        public Func<string> GetInnerHtml { get; set; } = () => "";
    }

    private string SetExtra(ReportType reportType)
    {
        return reportType == ReportType.PaymentReceipt || reportType == ReportType.ProductReturns ? @"
            <p style=""font-weight: bold; text-align: right; margin: 0; font-size: x-large;"">
                TOTAL: $ %TotalUsd%
            </p>
        " : "";
    }
    ...
    private class MyStringBuilder
    {
        ...
        public void AppendLine(string line) => Content += line;
    }

    public virtual void AddReportTokensInDefaultTemplateAsync(Report report, MessageTemplate defultMessageTemplate, MessageTemplate messageTemplate, IList<Token> tokens)
    {
        if (report.DataDic is null)
        {
            throw new NopException("Report data is null in AddReportTokensInDefaultTemplateAsync");
        }

        var sb = new MyStringBuilder();
        var originalMessageTemplateBody = messageTemplate.Body;
        messageTemplate.Body = defultMessageTemplate.Body;

        //DIVs
        var extraIdsList = new List<string>() { "root", "before", "after", "header", "footer", "title", "extra" };
        var htmlTokensConstList = extraIdsList.Select<string, HtmlTokenConst>(x => new()
        {
            Id = x,
            GetInnerHtml =
                x == "root"
                ? () => sb.ToString()
                : x == "extra"
                ? () => SetExtra(report.Type)
                : () => (GetInnerHtmlById(originalMessageTemplateBody, x) ?? "")
        }).ToList();

        var rootTokenConst = htmlTokensConstList.FirstOrDefault(x => x.Id == "root");
        if (rootTokenConst == null || (rootTokenConst != null && !defultMessageTemplate.Body.Contains(rootTokenConst.Span)))
        {
            throw new NopException("Invalid report defult template: No root div");
        }


        //Subject
        tokens.Add(new Token("Subject", messageTemplate.Subject));
        var documentMainTitle = messageTemplate.Subject.Split(" - ")[0].Trim();
        tokens.Add(new Token("DocumentMainTitle", documentMainTitle));
        //Report data
        tokens.Add(new Token("Report.Id", report.Id));
        tokens.Add(new Token("Report.CustomerId", report.CustomerId));
        //get last 10 characters of the ExtId
        string extId = report.ExtId == null ? "" : report.ExtId.Length >= 8 ? report.ExtId[^8..] : report.ExtId;
        tokens.Add(new Token("Report.ExtId", extId));

        //Data
        bool openFieldsTable = false;
        int index = 0;
        ...
        foreach (var item in report.DataDic)
        {
            ...
            else if (item.StringValue != null)
            {
                ...
                if (!openFieldsTable)
                {
                    openFieldsTable = true;
                    sb.AppendLine("<table class=\"field_container\">");
                }
                ...
                sb.AppendLine($@"
                    <td class=""field_cell"">
                        <p class=""field_key"">{item.Label}:</p>
                        <p class=""field_value"">{item.StringValue}</p>
                    </td>
                ");
        ...
        foreach (var htmlTokenConst in htmlTokensConstList)
        {
            messageTemplate.Body = messageTemplate.Body.Replace(htmlTokenConst.Span, htmlTokenConst.GetInnerHtml());
        }
    ...
    private string GetBeginHtml(string? title = null)
    {
        return @$"
                <html>
                    <head>
                        ...
                        <style>
                           /* Estilos para evitar que ciertos elementos se=
                            impriman */
                            @media print {{
                                ...
                                /* Por ejemplo, no imprimir el encabezado y=
                                    pie de pagina */
                                header,
                                footer {{
                                    display: none;
    ...
    private void SurroundTemplate(MessageTemplate messageTemplate)
    {
        messageTemplate.Body = GetBeginHtml(messageTemplate.Subject) + messageTemplate.Body + GetEndHtml();
    }

    private void GetTable(ReportData item, MyStringBuilder sb)
    {
        if (item.TableValue == null)
        {
            return;
        }
        ...
        //Begin Table
        sb.AppendLine("<table class=\"table_container\">");

        //Begin Headers
        sb.AppendLine($"<tr class=\"table_titles\">");
        foreach (string column in item.TableValue.Headers)
        {
            sb.AppendLine(@$"
                        <th class=""table_cell"">
                            {column}
        ...
        //Begin Rows
        foreach (var rowList in item.TableValue.Rows)
        {
            sb.AppendLine($"<tr class=\"table_body\">");

            foreach (var row in rowList)
            {
                sb.AppendLine("<td class=\"table_cell\">" + row + "</td>");
            }
    ...
    private void GetStyledTable(string label, StyledTable styledTable, MyStringBuilder sb)
    {
        //Label
        sb.AppendLine(@$"
                <table class=""table_title"">
        ...
        //Begin Table
        sb.AppendLine($"<table style=\"{styledTable.TableStyle}\">");

        //Begin Headers
        sb.AppendLine($"<tr style=\"{styledTable.HeaderRowStyle}\">");
        foreach (var column in styledTable.Headers)
        {
            sb.AppendLine(@$"
        ...
        {
            sb.AppendLine($"<tr style=\"{row.Style}\">");

            foreach (var cell in row.Cells)
            {
                sb.AppendLine($"<td style=\"{cell.Style}\">{cell.Value}</td>");
            }
    ...
    private void AddCommonTokens(MessageTemplate messageTemplate, Report report, IList<Token> tokens)
    {
        //Subject
        tokens.Add(new Token("Subject", messageTemplate.Subject));
        var documentMainTitle = messageTemplate.Subject.Split(" - ")[0].Trim();
        ...
        tokens.Add(new Token("Report.ExtId", extId));
        //date
        tokens.Add(new("Server.CurrentDateRelative", DateTime.UtcNow.AddHours(-4).ToString("dd/MM/yyyy")));
