...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <param name="order">Order</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task SendNotificationsAndSaveNotesAsync(Order order)
        {
            //notes, messages
        ...
        /*
        Paso1: Obtener numero de articulos del carrito
        Paso2: Vaciar el carrito
        Paso3: clonar processPaymentRequest
        Paso4: Meter 3 elementos en el carrito
        Paso3: repetir el proceso de PlaceOrderAsync por cada clon de processPaymentRequest
        Paso4: retornar una lista de PlaceOrderResult
        */

        /// <summary>
        /// Places an order with limit 3
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains a list of place order results
        /// </returns>
        public virtual async Task<IList<PlaceOrderResult>> PlaceOrderAsyncLimit3(ProcessPaymentRequest processPaymentRequestOriginal)
        {

            if (processPaymentRequestOriginal == null)
                throw new ArgumentNullException(nameof(processPaymentRequestOriginal));

            //Get client
            var customer = await _customerService.GetCustomerByIdAsync(processPaymentRequestOriginal.CustomerId);

            //Get shopping cart items
            IList<ShoppingCartItem> cart = await _shoppingCartService.GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, processPaymentRequestOriginal.StoreId);

            //Make a result list
            IList<PlaceOrderResult> resultList = new List<PlaceOrderResult>();

            //Make a taxes list
            IList<decimal> taxesList = new List<decimal>();

            //Make a shoppingCartItems list
            IList<string> cartSectionList = new List<string>();

            //Make a shoppingCartItems list
            IList<string> cartSectionList2 = new List<string>();

            if (!cart.Any())
                throw new NopException("Cart is empty");

            int orderQuantity = cart.Count / _orderSettings.MaxItemsPerOrder + (cart.Count % _orderSettings.MaxItemsPerOrder > 0 ? 1 : 0);
            ...
            {
                //Clone processPaymentRequest
                var processPaymentRequest = new ProcessPaymentRequest
                {
                    CustomerId = processPaymentRequestOriginal.CustomerId,
                    StoreId = processPaymentRequestOriginal.StoreId,
                    OrderGuid = Guid.NewGuid(),
                    OrderGuidGeneratedOnUtc = DateTime.UtcNow,
                    PaymentMethodSystemName = processPaymentRequestOriginal.PaymentMethodSystemName,
                    CustomValues = processPaymentRequestOriginal.CustomValues,
                    // CreditCardCvv2 = processPaymentRequestOriginal.CreditCardCvv2,
                    // CreditCardExpireMonth = processPaymentRequestOriginal.CreditCardExpireMonth,
                    // CreditCardExpireYear = processPaymentRequestOriginal.CreditCardExpireYear,
                    // CreditCardName = processPaymentRequestOriginal.CreditCardName,
                    // CreditCardNumber = processPaymentRequestOriginal.CreditCardNumber,
                    // CreditCardType = processPaymentRequestOriginal.CreditCardType,
                    // InitialOrder = processPaymentRequestOriginal.InitialOrder,
                    // OrderTotal = processPaymentRequestOriginal.OrderTotal,
                    // RecurringCycleLength = processPaymentRequestOriginal.RecurringCycleLength,
                    // RecurringCyclePeriod = processPaymentRequestOriginal.RecurringCyclePeriod,
                    // RecurringTotalCycles = processPaymentRequestOriginal.RecurringTotalCycles,
                };

                var result = new PlaceOrderResult();

                //Get a cart section
                IList<ShoppingCartItem> cartSection = cart.Skip(i * _orderSettings.MaxItemsPerOrder).Take(_orderSettings.MaxItemsPerOrder).ToList();

                //Save orderTotal
                // var (orderTotal, _, _, _, _, _) = _orderTotalCalculationService.GetShoppingCartTotalAsync
                // processPaymentRequest.OrderTotal = orderTotal;

                //Fill cart with cartSection
                foreach (var sci in cartSection)
                {
                    var product = await _productService.GetProductByIdAsync(sci.ProductId);
                    await _shoppingCartService.AddToCartAsync(customer, product, ShoppingCartType.ShoppingCart, processPaymentRequestOriginal.StoreId);
                }

                /*
                AddToCartAsync(Customer customer, Product product,
                ...
                */
                ...
                //IList<ShoppingCartItem> cartSection2 = await _shoppingCartService.GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, processPaymentRequestOriginal.StoreId);

                //Tax
                //var tax = await _taxService.GetTaxTotalAsync(cartSection2);
                //taxesList.Add(tax.TaxTotal);

                //cartSectionList
                //cartSectionList.Add(string.Join(",", cartSection.Select(sci => sci.ProductId).ToList()));

                //cartSectionList2
                //cartSectionList2.Add(string.Join(",", cartSection2.Select(sci => sci.ProductId).ToList()));

                result = await PlaceOrderAsync(processPaymentRequest);

                resultList.Add(result);
            }

            //if some result is not sucess return all items to cart and delete all sucess orders
            if (resultList.Any(resultList => !resultList.Success))
            {
                //Empty cart
                cart.ToList().ForEach(async sci => await _shoppingCartService.DeleteShoppingCartItemAsync(sci, false));
                ...
                //Delete all success orders
                foreach (var result in resultList.Where(r => r.Success))
                {
                    await _orderService.DeleteOrderAsync(result.PlacedOrder);
                }
            }


            //throw new NopException("[taxesList " + string.Join(",", taxesList) + "] [cartSectionList " + string.Join(",", cartSectionList) + "] [cartSectionList2 " + string.Join(",", cartSectionList2) + "]");
