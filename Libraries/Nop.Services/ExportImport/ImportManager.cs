...
#nullable enable
        ...
        /// <param name="stream">Stream</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task<(List<SkuPicture> productsUpdatedSP, List<SkuPicture> productsRejectedSP, List<Product> productsUpdated)> ImportProductsPicturesFromSkuPictureAsync(IList<SkuPicture> skuPictureList)
        {

            var downloadedFiles = new List<string>();

            var allSku = skuPictureList.Select(x => x.Sku).ToList();

            //performance optimization, load all products by SKU in one SQL request
            var allProductsBySku = await _productService.GetProductsBySkuAsync(allSku.ToArray(), 0);

            //product to import images
            var productPictureMetadata = new List<ProductPictureMetadata>();

            var productsUpdatedSP = new List<SkuPicture>();
            var productsUpdated = new List<Product>();
            var productsRejectedSP = new List<SkuPicture>();

            foreach (var skuPicture in skuPictureList)
            {
                var product = allProductsBySku.FirstOrDefault(p => p.Sku == skuPicture.Sku);

                if (product == null)
                {
                    productsRejectedSP.Add(new SkuPicture(null, skuPicture.Sku, null, null, null, "Product not found in DB"));
                    continue;
                }


                string path = _fileProvider.MapPath("~/wwwroot/images/siclab_images");
                ...
                //copy fileNames
                List<string> pictureFullPath = new(3)
                {
                    "",
                    "",
                    ""
                ...
                {
                    pictureFullPath[i] = Path.Combine(path, fileNames[i]);
                    var extension = Path.GetExtension(pictureFullPath[i]);

                    //If the file not exists
                    if (fileNames[i] == "" || !File.Exists(pictureFullPath[i]) || (extension != ".jpg" && extension != ".png"))
                    {
                        pictureFullPath[i] = "";
                    }
                }

                if (pictureFullPath.Any(p => p != ""))
                {
                    productPictureMetadata.Add(new ProductPictureMetadata
                    {
                        ProductItem = product,
                        Picture1Path = pictureFullPath[0],
                        Picture2Path = pictureFullPath[1],
                        Picture3Path = pictureFullPath[2],
                        IsNew = false
                    });

                    productsUpdatedSP.Add(new SkuPicture(product.Id, product.Sku, pictureFullPath[0], pictureFullPath[1], pictureFullPath[2]));

                    productsUpdated.Add(product);
                }
                else
                {
                    productsRejectedSP.Add(new SkuPicture(product.Id, product.Sku, null, null, null, "picture file is not valid"));
                }
            }
            ...
            //activity log
            await _customerActivityService.InsertActivityAsync("ImportProducts", string.Format(await _localizationService.GetResourceAsync("ActivityLog.ImportProducts"), productsUpdatedSP.Count));

            return (productsUpdatedSP, productsRejectedSP, productsUpdated);
        }

        public class SkuPicture
        {
            public SkuPicture(int? id, string sku, string? picture1, string? picture2, string? picture3, string? comment = null)
            {
                Id = id;
                Sku = sku;
                Picture1 = picture1;
